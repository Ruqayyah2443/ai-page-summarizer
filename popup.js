...
//   // Add this right after you get the tab
// const [tab] = await chrome.tabs.query({ active: true, currentWindow: true });

// // ADD THESE LINES:
// if (!tab.url || (!tab.url.startsWith("http://") && !tab.url.startsWith("https://"))) {
//   document.getElementById('page-title').textContent = "Not a webpage";
//   document.getElementById('summarize-btn').disabled = true;
//   document.getElementById('summarize-btn').textContent = "Open a website first";
//   return;
// }
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  document.getElementById('summarize-btn').addEventListener('click', async () => {
    ...
    try {
      ...
      if (!response) throw new Error("No response from content script. Refresh and try again.");
      if (!response.success) throw new Error("Content script failed: " + response.error);
      if (!response.content || response.content.length < 20) throw new Error("Page has no readable content.");
  ...
  // ✅ CLEAR BUTTON FIX — resets ALL elements properly
  document.getElementById('clear-btn').addEventListener('click', () => {
    document.getElementById('summary-output').textContent = '';
    document.getElementById('error').textContent = '';
    document.getElementById('clear-btn').style.display = 'none';
    document.getElementById('summarize-btn').style.display = 'block';
    document.getElementById('summarize-btn').disabled = false; // ← THIS was the missing line
  });
